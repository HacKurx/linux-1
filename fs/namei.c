...
struct dentry *cow_break_link(const char *pathname)
{
	...
	vxdprintk(VXD_CBIT(misc, 1), "cow_break_link(" VS_Q("%s") ")", pathname);
	...
	ret = filename_lookup(AT_FDCWD, filename, LOOKUP_FOLLOW,
			      &old_path, NULL);
...
retry:
	...
	vxdprintk(VXD_CBIT(misc, 2), "vfs_create(new): %d", ret);
	if (ret == -EEXIST) {
		path_put(&par_path);
		inode_unlock(dir->d_inode);
	...
	vxdprintk(VXD_CBIT(misc, 2), "dentry_open(old): %p", old_file);
	if (IS_ERR(old_file)) {
	...
	vxdprintk(VXD_CBIT(misc, 2), "dentry_open(new): %p", new_file);
	if (IS_ERR(new_file)) {
