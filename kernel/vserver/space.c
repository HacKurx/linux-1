...
atomic_t vs_global_cgroup_ns	= ATOMIC_INIT(0);
	...
	struct nsproxy *new_nsproxy, unsigned long mask)
{
	...
	struct cgroup_namespace *old_cgroup;
#ifdef	CONFIG_PID_NS
	...
	if (mask & CLONE_NEWCGROUP) {
		old_cgroup = nsproxy->cgroup_ns;
		nsproxy->cgroup_ns = new_nsproxy->cgroup_ns;
